...
def plot_behavior_comparison(scenarios):
    ...
    for behavior, scenarios_list in behavior_groups.items():
        ...
        for name, df, params in scenarios_list:
            ...
            # Find recovery time (when efficiency > 95%)
            recovery_idx = df[df['eficiencia_pct'] > 95].index
            if len(recovery_idx) > 0:
                recovery_time = df.loc[recovery_idx[0], 'segundo']
...
def plot_recovery_analysis(scenarios):
    ...
    for name, df in scenarios.items():
        ...
        # Find when jam dissolves (efficiency > 90% and stable)
        high_eff = df[df['eficiencia_pct'] > 90]
        if len(high_eff) > 5:
            recovery_time = high_eff.iloc[0]['segundo']
        else:
    ...
    recovery_df = recovery_df[recovery_df['recovery_time'].notna()]
    ...
    if 'num_cars' in recovery_df.columns:
        for behavior_type in recovery_df['type'].unique():
            subset = recovery_df[recovery_df['type'] == behavior_type]
    ...
    axes[0, 1].scatter(recovery_df['max_stopped'], recovery_df['recovery_time'],
                      s=150, alpha=0.7, c=recovery_df['num_cars'], cmap='viridis')
    ...
    axes[1, 0].scatter(recovery_df['total_energy'], recovery_df['recovery_time'],
                      s=150, alpha=0.7, c=recovery_df['max_stopped'], cmap='Reds')
    ...
    behavior_types = recovery_df['type'].unique()
    data_by_type = [recovery_df[recovery_df['type'] == bt]['recovery_time'].values 
    ...
    plt.suptitle('Jam Recovery Analysis: What determines recovery speed?', 
                fontsize=15, fontweight='bold')
    plt.tight_layout()
...
def plot_summary_statistics(scenarios):
    ...
    for name, df in scenarios.items():
        ...
        # Recovery time
        high_eff = df[df['eficiencia_pct'] > 90]
        recovery_time = high_eff.iloc[0]['segundo'] if len(high_eff) > 0 else np.nan
        
        # Average during jam (t=15-25)
        jam_period = df[(df['segundo'] >= 15) & (df['segundo'] <= 25)]
        avg_vel_jam = jam_period['vel_media'].mean()
        
        summary_data.append({
            'Scenario': name,
            'Type': params.get('type', 'Other'),
            'Density (N)': params.get('num_cars', 0),
    ...
    summary_df = pd.DataFrame(summary_data)
    summary_df = summary_df.sort_values('Recovery Time (s)')
    ...
    # Bar chart: Recovery time
    top10_recovery = summary_df.nsmallest(10, 'Recovery Time (s)')
    ...
    # Scatter: Recovery time vs max stopped
    axes[1, 0].scatter(summary_df['Max Stopped'], summary_df['Recovery Time (s)'],
                      s=150, alpha=0.7, c=summary_df['Density (N)'], cmap='plasma')
